...
| chunker_type | String | 否 | "smart" | 分块器类型：smart, law, audit, speech_material, case_material, technical_standard, default |
| doc_type | String | 否 | "internal_regulation" | 文档类型：internal_regulation, external_regulation, internal_report, external_report；纪检域会优先按 `knowledge_labels.library` 自动派生 |
...
| chunker_type | String | 否 | "smart" | 分块器类型：smart, law, audit, speech_material, case_material, technical_standard, issue, default |
| doc_type | String | 否 | "internal_regulation" | 文档类型：internal_regulation, external_regulation, internal_report, external_report, audit_issue；纪检域会优先按 `knowledge_labels.library` 自动派生 |
| save_after_processing | String | 否 | "true" | 处理后是否自动保存向量库 |
| scope | String | 否 | default_scope | 知识域：audit 或 discipline |
...
对于几百份文件的生产环境初始化导库，建议不要在浏览器里反复点传，而是按分类目录分批导入：

```text
/data/discipline_import/
├── 重要讲话精神/
├── 国家法律法规/
├── 常用党内法规/
└── 典型案例库/
...
```bash
python3 tools/batch_import_archives.py \
  --base-url http://127.0.0.1:8000 \
  --scope discipline \
  --input-dir /data/discipline_import \
  --group-mode subdir \
  --classification-key library \
  --label-map-json '{"重要讲话精神":"important_speeches","国家法律法规":"national_laws","常用党内法规":"party_regulations","典型案例库":"case_library"}' \
  --doc-type internal_regulation \
  --chunker-type smart \
  --batch-max-files 80 \
  --batch-max-source-bytes 125829120
```

说明：

- `group-mode=subdir`：按一级子目录分组，每个目录会拆成多个 ZIP 批次
- `classification-key=library`：把目录映射到知识分类字段
- `label-map-json`：把目录名映射成后端实际使用的分类值
- 脚本会输出每批成功/失败信息，并生成一份 JSON 报告文件
- 默认会跳过非 `PDF/DOCX/TXT` 文件，以及超过服务端单文件限制的文件
- 如生产环境有鉴权，可增加 `--auth-token` 参数
...
curl -X POST http://localhost:8000/upload_archive_store \
  ...
  -F "archive=@/path/to/典型案例库.zip" \
  -F "chunker_type=case_material" \
  -F 'knowledge_labels={"library":["case_library"]}'
```
...
curl -X POST http://localhost:8000/upload_archive_store \
  -H "X-Knowledge-Scope: discipline" \
  -F "archive=@/path/to/重要讲话精神.zip" \
  -F "chunker_type=speech_material" \
  -F 'knowledge_labels={"library":["important_speeches"]}'
```
