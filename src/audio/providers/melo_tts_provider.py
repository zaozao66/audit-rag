...
class MeloTTSProvider(BaseTTSProvider):
    ...
    def synthesize(self, request: TTSRequest) -> TTSResult:
        if not self.endpoint:
            raise ValueError("MeloTTS endpoint 未配置")
        ...
        payload = {
            ...
            "speaker": request.voice or self.default_voice,
            "format": audio_format,
        ...
        if response.status_code >= 400:
            raise RuntimeError(f"MeloTTS 请求失败({response.status_code}): {response.text[:220]}")
    ...
    def _decode_audio_json(self, payload_text: str) -> bytes:
        ...
        for key in ("audio", "audio_base64", "b64_audio"):
            ...
            try:
                ...
                continue
        raise RuntimeError("MeloTTS JSON中未找到可解码音频")
