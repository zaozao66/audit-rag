...
10. **多知识域隔离**：支持 `audit`（审计）与 `discipline`（纪检）知识域隔离，文档/检索/会话互不串线

前端说明：
- 文档管理页已移除“全文预览”，仅保留“PDF全屏预览”
...
多知识域调用说明：
- 推荐通过 Header `X-Knowledge-Scope` 指定知识域，取值：`audit`、`discipline`
- 也支持在 query/body/form 中传 `scope`
- 未传 `scope` 时使用 `config.json` 的 `default_scope`
- 出于隔离安全，`store_path` 透传已禁用
...
- `scope`: (可选，推荐显式传) 知识域：`audit` 或 `discipline`
...
多知识域推荐补充字段示例（可放在 `development` / `production` 下）：
...
```json
{
  ...
  "knowledge_scopes": {
    ...
    "discipline": {
      "vector_store_path": "./data/discipline/vector_store_text_embedding",
      "intent_router": {
        "enabled": false,
        "fixed_top_k": 8
      }
...
- `development`: 开发环境配置
  ...
  - `vector_store_path`: 向量库存储路径（推荐配合 `knowledge_scopes` 按知识域隔离）
  - `default_scope`: 默认知识域
  - `scope_required`: 是否强制请求必须带 scope
  - `knowledge_scopes`: 多知识域配置（如 audit/discipline）
